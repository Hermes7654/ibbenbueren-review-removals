...
func run(args args) error {
	...
	for _, chart := range charts {
		...
		base := "ibbenbueren_overall_summary"
		if chart.Scope != "overall" {
			base = "ibbenbueren_" + chart.Scope + "_summary"
		}
...
func writeMostRemovedList(rows []mapsreview.Place, outDir string) error {
	...
	if err := writeMostRemovedCSV(filepath.Join(outDir, "ibbenbueren_most_removed.csv"), ranked); err != nil {
		return err
	}
	if err := writeMostRemovedMD(filepath.Join(outDir, "ibbenbueren_most_removed.md"), ranked); err != nil {
		return err
	}
	if err := writeMostRemovedHTML(filepath.Join(outDir, "ibbenbueren_most_removed.html"), ranked, rows); err != nil {
		return err
	}
	fmt.Printf("wrote %s\n", filepath.Join(outDir, "ibbenbueren_most_removed.csv"))
	fmt.Printf("wrote %s\n", filepath.Join(outDir, "ibbenbueren_most_removed.md"))
	fmt.Printf("wrote %s\n", filepath.Join(outDir, "ibbenbueren_most_removed.html"))
	return nil
}
...
func writeMostRemovedMD(file string, rows []mapsreview.Place) error {
	lines := []string{
		"# Ibbenbüren — Orte sortiert nach geschätzter Anzahl entfernter Bewertungen",
		"",
